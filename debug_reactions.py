...
what reaction-related fields exist in the feedback node."""
    ...
    async with httpx.AsyncClient() as client:
        ...
        for line in text.split("\n"):
            ...
            try:
                ...
                for i, edge in enumerate(comments[:3]):  # first 3 comments
                    node = edge.get("node", {})
                    ...
                    print(f"Feedback keys: {list(fb.keys())}")
                    
                    # Check for top_reactions
                    if "top_reactions" in fb:
                        print(f"top_reactions: {json.dumps(fb['top_reactions'], indent=2)}")
                    else:
                        print("top_reactions: NOT PRESENT")
                    
                    # Print reactors
                    reactors = fb.get("reactors", {})
                    print(f"reactors: {json.dumps(reactors, indent=2)}")
                    
                    # Print ALL feedback keys with values
                    print(f"\nFull feedback dump:")
