...
async def test():
    ...
    async with httpx.AsyncClient(cookies=cookies, follow_redirects=True) as client:
        resp = await client.get("https://www.facebook.com/photo?fbid=1532133465628571&set=a.650104283831498", headers=headers)
        
        with open("test_page.html", "w", encoding="utf-8") as f:
            f.write(resp.text)
            
        print("Length:", len(resp.text))
        print("Is login page?", "login_form" in resp.text)
        print("Contains true feedback ID?", "ZmVlZGJhY2s6MTUzMjEzMzgzODk2MTg2Nw==" in resp.text)
